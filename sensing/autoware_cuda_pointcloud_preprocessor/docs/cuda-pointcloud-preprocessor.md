...
- This node expects that the input pointcloud follows the `autoware::point_types::PointXYZIRCAEDT` layout and preserves that layout in the output pointcloud so downstream raw-ex consumers can keep the original per-point metadata.
